  ...
  const handleDashboard = async () => {
    ...
    try {
      ...
      if (appointmentsRes.status === "fulfilled") {
        const recentAppointment = (appointmentsRes.value?.data?.appointments ?? [])
              ...
              <CardContent key={item._id} className="flex justify-between gap-6">
                <div className="flex flex-col md:flex-row gap-4">
                  <Avatar className="size-20 border border-slate-100 bg-white">
                    <AvatarFallback>
                      {`${item?.appointmentId?.providerId?.firstName?.[0] || ""}${item?.appointmentId?.providerId?.lastName?.[0] || ""
                        }`}
                    </AvatarFallback>
                  </Avatar>
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex flex-col text-sm text-muted-foreground">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              {dayjs(item?.appointmentId?.date).format("MMMM DD, YYYY")}
                            </span>
                          </div>
          ...
          {dashboardData?.nextAppointmentLoading ? (
            ...
            <CardContent>
              <div className="space-y-1">
                ...
                <div className="text-sm ">{dashboardData?.mentalPlans?.[0]?.title}</div>
              </div>

              <div className="mt-4">
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <div>Progress</div>
                  <div className="font-medium">{dashboardData?.mentalPlans?.[0]?.progress}%</div>
                </div>

                <div className="mt-2 h-2 bg-muted rounded-lg overflow-hidden">
                  <div
                    className="h-full bg-gradient-dash"
                    style={{ width: `${dashboardData?.mentalPlans?.[0]?.progress}%` }}
                  />
                </div>
                ...
                <div className="mt-3 text-sm text-muted-foreground">
                  ...
                  <span className="font-medium">
                    {dashboardData?.mentalPlans?.[0]?.completedSessions} of {dashboardData?.mentalPlans?.[0]?.totalSessions} completed
                  </span>
                </div>
              </div>
            </CardContent>
                ...
                <tbody className="text-sm">
                  {dashboardData?.recentAppointment?.slice(0, 3).map((item: any, index: number) => (
                    <tr key={item._id} className="border-b">
                      <td className="py-4 px-2 pl-4">
                        Dr. {item?.providerId?.firstName} {item?.providerId?.lastName}
                      </td>
